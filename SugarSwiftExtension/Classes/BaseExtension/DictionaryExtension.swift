...
extension Dictionary{
    ...
    ///  Intersection of self and the input dictionarie.
    /// Two dictionaries are considered equal if they contain the same [key: value] copules.
    public func intersection<K:Hashable, V: Equatable>(_ dictionaries: [K: V]...) -> [K: V] {
        ...
        //  Intersection
        var result = filtered
        var intersection = [K: V]()
        dictionaries.forEach { (dic) in
            dic.forEach { (item) in
                let contains = result.contains(where: { (k,v) -> Bool in
                    return item.key == k && item.value == v
                })
                if contains {
                    intersection.appendItem(element: item)
                }
            }
    ...
    /// appendItem
    public mutating func appendItem(element:Element){
